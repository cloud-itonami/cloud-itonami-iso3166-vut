...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (VUT, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up low-value-procurement-ineligible) ==")
    ...
    (println "== filing/submit eng-4 (low-value-procurement-ineligible -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tax-clearance-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tax-clearance-record-unverified -> HARD hold) ==")
