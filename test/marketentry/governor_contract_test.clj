  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Port Vila Trading Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Port Vila Trading Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest low-value-procurement-ineligible-is-held-and-unoverridable
  (testing "declared :claims-low-value-procurement? true while the engagement's own declared contract value sits at/above VT 10,000,000 -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:low-value-procurement-ineligible} (-> (store/ledger db) last :basis)))
...
(deftest tax-clearance-record-unverified-is-held-and-unoverridable
  (testing "unverified tax clearance record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tax-clearance-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
