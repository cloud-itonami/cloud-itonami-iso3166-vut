(ns marketentry.marketentryllm
  ...
  the Republic of Vanuatu (VUT) public-sector market-entry compliance
  actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "claims-low-value-procurement?=" (:claims-low-value-procurement? e)
                        " declared-contract-value=" (:declared-contract-value e)
                        " tax-clearance-record-verified?=" (:tax-clearance-record-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-tax-clearance-record? e))
                              (:tax-clearance-record-verified? e)))
                   0.9 0.3)}))
